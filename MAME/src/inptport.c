...
InputSeq* input_port_seq_copy_default(struct InputPort *in, int seq)
{
	int i;
	UINT32 type;
	...
	memcpy( &in->seq[seq], &inputport_defaults[i + seq].seq, sizeof( InputSeq ) ) ;
...
{
	struct InputPort *in;
	int current, delta=0, sensitivity, min, max, default_value;
