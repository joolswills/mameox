...
#else
#define GTELOG( a )
//#define GTELOGX( a, ... )   // [EBA] This is not a standard extension...
#endif
...
//	GTELOGX( "set CP2CR%u=%08x", n_reg, n_value );
  ...
  // [EBA] Use the INLINE macro, "static inline" is not always correct
INLINE INT32 LIM( INT32 n_value, INT32 n_max, INT32 n_min, int n_bit )
{
	if( n_value > n_max )
	{
  ...
  // [EBA] Use the INLINE macro, "static inline" is not always correct
INLINE INT64 BOUNDS( INT64 n_value, INT64 n_max, int n_maxbit, INT64 n_min, int n_minbit )
{
	if( n_value > n_max )
	{
		FLAG |= ( 1 << n_maxbit );
