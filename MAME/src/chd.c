...
struct chd_file *chd_open(const char *filename, int writeable, struct chd_file *parent)
{
	...
	chd.cache = osd_malloc(chd.header.hunkbytes);
	chd.compare = osd_malloc(chd.header.hunkbytes);
	...
	chd.compressed = osd_malloc(chd.header.hunkbytes);
	if (!chd.compressed)
		SET_ERROR_AND_CLEANUP(CHDERR_OUT_OF_MEMORY);
	...
	finalchd = osd_malloc(sizeof(chd));
	if (!finalchd)
		SET_ERROR_AND_CLEANUP(CHDERR_OUT_OF_MEMORY);
	*finalchd = chd;
...
static int read_hunk_map(struct chd_file *chd)
{
	...
	chd->map = osd_malloc(sizeof(chd->map[0]) * chd->header.totalhunks);
	if (!chd->map)
...
static void init_crcmap(struct chd_file *chd, int prepopulate)
{
	...
	chd->crcmap = osd_malloc(chd->header.totalhunks * sizeof(chd->crcmap[0]));
	if (!chd->crcmap)
		return;
	
	/* allocate a CRC map table */
	chd->crctable = osd_malloc(CRCMAP_HASH_SIZE * sizeof(chd->crctable[0]));
	if (!chd->crctable)
	{
		free(chd->crcmap);
		chd->crcmap = NULL;
...
struct chd_exfile *chd_start_compress_ex(struct chd_file *chd)
{
	...
	finalchdex = osd_malloc(sizeof(struct chd_exfile));
	if (!finalchdex)
		SET_ERROR_AND_CLEANUP(CHDERR_OUT_OF_MEMORY);
...
static int init_codec(struct chd_file *chd)
{
	...
	switch (chd->header.compression)
	{
		...
		case CHDCOMPRESSION_ZLIB_PLUS:
		{
			...
			chd->codecdata = osd_malloc(sizeof(struct zlib_codec_data));
			if (!chd->codecdata)
				return CHDERR_OUT_OF_MEMORY;
